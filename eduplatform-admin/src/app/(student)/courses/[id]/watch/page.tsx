...
function CourseWatchContent() {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-slate-900 text-white font-black animate-pulse uppercase tracking-[0.3em]">Eğitim Yükleniyor...</div>
  ...
  return (
    <div className="flex h-screen bg-[#0F172A] overflow-hidden text-slate-200">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        {/* Top Header/Bar */}
        <header className="h-16 flex items-center justify-between px-6 bg-[#0F172A] border-b border-slate-800 shrink-0">
          <div className="flex items-center gap-4">
             <Link href={`/courses/${id}`} className="p-2 hover:bg-slate-800 rounded-xl transition-colors">
                <ArrowLeft size={20} />
             </Link>
             <div className="flex flex-col">
                <span className="text-[10px] uppercase font-black tracking-widest text-slate-500">Şu An İzlenen</span>
                <span className="text-sm font-bold text-white line-clamp-1">{course?.title}</span>
             </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar p-6 lg:p-10">
            <div className="max-w-5xl mx-auto space-y-10">
                {/* Visual Area */}
                <div className="relative aspect-video bg-black rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/5 ring-1 ring-white/10 group">
                    {currentLesson?.content_type === "video" ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center bg-slate-800/50 backdrop-blur-3xl p-12 text-center">
                            <div className="w-20 h-20 rounded-3xl bg-primary/20 text-primary flex items-center justify-center mb-6">
                                <FileText size={40} />
                            </div>
                            <h2 className="text-3xl font-black text-white mb-4">{currentLesson?.title}</h2>
                            <p className="text-slate-400 max-w-md">Bu ders metin bazlı bir içeriktir. İçeriği aşağıdan okuyabilirsiniz.</p>
                        </div>
                    )}
                ...
                <div className="space-y-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-8">
                        <div className="space-y-2">
                             <div className="flex items-center gap-3">
                                <Badge className="bg-primary/20 text-primary border-none font-black text-[10px] px-3 py-1 rounded-full uppercase tracking-widest">{lessons.indexOf(currentLesson) + 1}. Ders</Badge>
                                <div className="text-xs text-slate-500 font-bold uppercase tracking-tighter">İçerik Yayında</div>
                             </div>
                             <h1 className="text-3xl font-black text-white tracking-tight">{currentLesson?.title}</h1>
                        </div>
                        <div className="flex items-center gap-3">
                            <Button variant="outline" className="h-12 border-slate-700 bg-slate-800/50 hover:bg-slate-700 font-bold rounded-2xl px-6 gap-2 text-white">
                    ...
                    <article className="prose prose-invert max-w-none text-slate-400 leading-relaxed text-lg">
                        {currentLesson?.content_markdown ? (
                            <ReactMarkdown>{currentLesson.content_markdown}</ReactMarkdown>
                        ) : (
                            <p>Bu ders için ek açıklama bulunmuyor.</p>
                        )}
                    </article>
                </div>
            </div>
        ...
        {/* Footer Navigation */}
        <footer className="h-20 bg-[#0F172A] border-t border-slate-800 flex items-center justify-between px-8 shrink-0">
            <button 
      ...
      {/* Sidebar - Curriculum */}
      <aside className={`w-[400px] bg-[#1E293B] flex flex-col shrink-0 transition-all duration-500 absolute lg:relative inset-y-0 right-0 z-[100] ${isSidebarOpen ? "translate-x-0" : "translate-x-full lg:w-0 lg:overflow-hidden"}`}>
         <div className="h-16 flex items-center justify-between px-6 bg-[#1E293B] border-b border-slate-700 shrink-0">
             <h3 className="font-black text-sm uppercase tracking-widest text-white">Eğitim Müfredatı</h3>
             <button onClick={() => setIsSidebarOpen(false)} className="p-2 hover:bg-slate-700 rounded-xl transition-colors">
         ...
         <div className="p-6 space-y-4">
            <div className="flex justify-between items-center mb-2">
                <span className="text-xs font-bold text-slate-400">Toplam İlerleme</span>
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2 mb-1">
                                <span className={`text-[10px] font-black uppercase tracking-widest ${isActive ? "text-primary" : "text-slate-500"}`}>{idx + 1}. Ders</span>
                                {isCompleted && <CheckCircle2 size={14} className="text-emerald-500" />}
                            </div>
                            <h4 className={`text-sm font-bold leading-snug line-clamp-2 ${isActive ? "text-white" : "text-slate-400"}`}>
                                {lesson.title}
                            </h4>
                            <div className="mt-2 text-[10px] font-bold text-slate-600 uppercase tracking-tighter">{lesson.content_type === "video" ? "VİDEO" : "METİN"}</div>
                        </div>
                    </div>
                 )
             })}
